    ...
    public partial class 지뢰찾기 : Form, IGameUI
    {
        ...
        private void 환경설정ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ( ServerFlag )
            {
                if (!SoloFlag)
                {
                    Alert alert = new Alert("대전중에는 난이도 변경을 할 수 없습니다.", "알림", "OK");
                    alert.Show();
                    return;
                }
                else
                {
                    Alert alert = new Alert("매칭은 기존의 선택한 난이도로 매칭됩니다.\n현재 변경되는 난이도는 싱글모드 또는 대기상태모드에만 적용됩니다.", "알림", "OK");
                    alert.Show();
                }       
            }
            ChooseLevel();
        ...
        private void 기권하기ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Alert alert = new Alert("기권 시 네트워크 연결이 종료됩니다. 싱글모드로 돌아가시겠습니까?", "알림", "YesOrNo");
        ...
        private async void 방나가기ToolStripMenuItem_ClickAsync(object sender, EventArgs e)
        {
            ...
            Alert alert = new Alert("현재 방에서 나가 대기방으로 가시겠습니까?\nYes or No", "방나가기", "YesOrNo");
