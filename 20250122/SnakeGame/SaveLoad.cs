...
namespace SnakeGame
{
    ...
    class SaveLoad
    {
        ...
        public SaveLoad(List<Point> snake, List<Point> obstacle, List<Point> feed, int lv, int length, int lvpoint, int jumsu, SnakeVector vector)
        {
            lbl_List[0] = snake;
            lbl_List[1] = obstacle;
            lbl_List[2] = feed;
        ...
        {  
            using (Stream rs = new FileStream(fileName, FileMode.Open))
            {
                ...
                for(int i = 0; i < 3; i++)
                {
                    var v = JsonSerializer.Deserialize<List<Point>>(str[i]);
                    lbl_List[i] = (List<Point>) v;
                }
               
                for (int i = 0; i < 4; i++)
                    state[i] = Convert.ToInt32(str[3+i]);

                if (str[7].Equals("UP"))
                    snakeVector = SnakeVector.UP;
                else if (str[7].Equals("DOWN"))
                    snakeVector = SnakeVector.DOWN;
                else if (str[7].Equals("LEFT"))

                    snakeVector = SnakeVector.LEFT;
                else if (str[7].Equals("RIGHT"))
