...
namespace Minesweeper
{
    public partial class 지뢰찾기 : Form
    {
        ...
        {
            network.SendMessage(COMMAND[4], mysock, oppsock, "click");
        }
        ...
        private void 지뢰찾기_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                int gameRsult = JudgeGameResult(myState, opp_state);
                if (gameRsult == -1) return;
                ...
                {
                    SendGameResult(gameRsult);
                }
                if (gameRsult == 1) GameOver(); // 패배한 경우
                else if (gameRsult == 2) GameClear(); // 클리어한 경우
            }
        }
