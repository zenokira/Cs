...
namespace Minesweeper
{
    ...
    */
    ...
    public partial class 지뢰찾기 : Form
    {
        ...
        void SoloGameInit()
        {
            FormResize(solo_level);
            InitGamePan(solo_level);
            RandomLayingMine(solo_level);
            DetectorMine(solo_level);

            gameoverFlag = false;
            SoloFlag = true;
            MineFlagCnt = gameManager.MineCount_COUNT[solo_level];
        ...
        private bool isGameClear(int level)
        {
            ...
            int openAmount = buttons.Length - gameManager.MineCount[level];
            if (openAmount == open)
                return true;
            return false;
        }
        ...
        void NetWorkGameInit()
        {
            ...
            MineFlagCnt = gameManager.MineCount_COUNT[multi_level];
            toolStripTextBox_MineFlag.Text = MineFlagCnt.ToString();
            timer = 0;
            toolStripTextBox_Timer.Text = "0";
            timer_PlayTime.Start();
        ...
        void HandleCloseRoomAction()
        {
            ...
            GameReset();
            ChooseLevel();
            network.SendMessage(CommandType.Client, mySocketNo, serverNo, ClientAction.Level, multi_level.ToString(), "CloseRoom");
            MultiGameInit();
            SetMystate();
        }
