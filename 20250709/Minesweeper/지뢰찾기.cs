    ...
    */
    ...
    public partial class 지뢰찾기 : Form
    {
        ...
        private void HandleSoloMode(Mine mine)
        {
            ...
            {
                OpenZeroButton(mine.GetButtonXYPoint(), gamemode_multi);
            }
            else
            {
                mine.ButtonClick();
        ...
        void HandleSystemCommand(int sendNo, int recvNo, string action, string data)
        {
            ...
            switch (action)
            {
                ...
                case "CloseRoom":
                    ...
                    break;
                case "Wait":
                    if (data.Equals("OppExit"))
                    {
                        if (alert != null)  alert.ForceClose();
                        alert = null;
                        Task.Run(() => MessageBox.Show("상대방이 나갔습니다"));         
                    }
                    HandleWaitAction();
                    break;
            }
        ...
        {
            Task.Run(() => MessageBox.Show("서버가 종료되어 싱글모드로 돌아갑니다.\n빠르게 재오픈 할 예정입니다"));
            ServerFlag = false;
            SoloFlag = true;
        ...
        void HandleResultCommand(int sendNo, int recvNo, string data)
        {
            ...
            string msg = $"{result}\n다시 하시려면 Yes 대기실로 돌아가시려면 No 를 눌러주세요";
            string title = $"Player{mySocketNo + 1} 결과";

            alert = new Alert(msg, $"Player{mySocketNo + 1} 결과");
            ...
            else if ( alert.DialogResult == DialogResult.No)
            {
        ...
        void HandleMineMapCommand(string data)
        {
            ...
            gameManager.MultiGameInit(buttons);
            NetWorkGameInit();
        }
