...
namespace Minesweeper
{
    ...
    */
    ...
    public enum SystemAction { Connect, Exit, Error, Disconnect, RoomClose, Wait };
    public partial class 지뢰찾기 : Form
    {
        ...
        private void HandlSololeMode(Mine mine)
        {
            if (mine.isMineHave())
            {
                GameOver();
            ...
            {
                DetectorZeroButton(buttons, mine.GetButtonXYPoint(), solo_level);
            }
            else
            {
                mine.ButtonClick();
