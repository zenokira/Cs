...
namespace MinesweeperServer
{
    ...
    {
        private const int ERROR_TRY_GET_VALUE = 5;
        private const int ERROR_TRY_REMOVE = 6;
        ...
        */
        ...
        private Dictionary<int, CancellationTokenSource> receiveTokens = new(); // Receive 루프 관리용
        ...
        void start()
        {
            ...
            while (true)
            {
                ...
                receiveTokens[player.Id] = cts;
                CancellationToken token = cts.Token;

                Task.Run(() => ProcessReceive(player, token));
            }
        }
        ...
        private void MatchingGame(Player player1, Player player2, int level)
        {
            ...
            string strMine = newRoom.MineConvertString();

            Console.WriteLine($"room level : {newRoom.Level} , room gamePan : {newRoom.GetGamePanSize()} mineLength : {strMine.Length} ");
           
            Player? p1 = null;
            Player? p2 = null;

            bool p1Exists = playerDict.TryGetValue(newRoom.Player1.Id, out p1);
            bool p2Exists = playerDict.TryGetValue(newRoom.Player2.Id, out p2);

            if (!p1Exists || !p2Exists)
            {
                socket_wait_level_list[level].RemoveAll(p => p == null);
                newRoom.DestroyRoom();

                if (p1 != null && !socket_wait_level_list[level].Contains(p1))
                    socket_wait_level_list[level].Add(p1);
                if (p2 != null && !socket_wait_level_list[level].Contains(p2))
                    socket_wait_level_list[level].Add(p2);

                return;
            }

            // 0. 매칭이 됐음을 알림
            Send(CommandType.Server, serverNo, player1.Id, ServerAction.Matching, "");
            Send(CommandType.Server, serverNo, player2.Id, ServerAction.Matching, "");
            ...
            // 2. 턴 정보 전송
            Send(CommandType.Server, player1.Id, player2.Id, ServerAction.TurnInfo, $"{newRoom.FirstTurnPlayer.Id}");
            Send(CommandType.Server, player2.Id, player1.Id, ServerAction.TurnInfo, $"{newRoom.FirstTurnPlayer.Id}");
            // 3. 지뢰맵 전송
        ...
        void ProcessReceive(Player player, CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested && player.IsConnected)
            {
                while (!token.IsCancellationRequested && player.IsConnected)
                {
                    try
                    {
                        ...
                        foreach (var message in list)
                        {
                            ...
                            // 명령 파싱
                            ParsingRecvData(message, out string command, out int sendNo, out int recvNo, out string action, out string data);
                    ...
                    catch (SocketException ex)
                    {
                        ...
                        {
                            // Receive 타임아웃: 루프 계속 돌면서 token 체크
                            continue;
                        }
                        else
                        {
        ...
        void CleanupPlayerConnection(Player player)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"[Server] 플레이어 {player.Id}의 소켓 연결을 서버에서 정리했습니다.");
                    Console.WriteLine($"[Server] 현재 접속중인 플레이어의 수는 {playerDict.Count} 입니다.");
                }
            }
            catch (Exception ex)
            {
        ...
        void RemovePlayer(Player player)
        {
            ...
            // 1. Receive 루프 종료
            if (receiveTokens.TryGetValue(player.Id, out var cts))
            {
                cts.Cancel();          // 루프 종료 신호
                cts.Dispose();         // TokenSource 정리
                receiveTokens.Remove(player.Id);
            }
            ...
            {
                playerDict.TryRemove(player.Id, out var _);
            }

            Console.WriteLine($"[{player.Id}] 플레이어 안전 제거 완료");
        }
