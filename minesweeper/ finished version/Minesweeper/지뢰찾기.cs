    ...
    */
    ...
    public partial class 지뢰찾기 : Form, IGameUI
    {
        ...
        private void 새로시작ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (GameInProgress)
            {
                Alert alert = new Alert("멀티 게임중에는 새로시작할 수 없습니다");
                alert.Show();
                return;
            }
            ButtonReset();
        ...
        private async void 시작하기ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (GameInProgress)
            {
                Alert alert = new Alert("멀티 게임중에는 불가능한 기능입니다.");
                alert.Show();
                return;
            }
            ...
            {
                Alert alert = new Alert("게임을 강제종료하려면 다음 게임을 준비하세요. 이전 게임을 하시겠습니까", "오류");
                alert.ShowDialog();
                return;
            }
            ServerFlag = true;
        ...
        public async void CallAsyncChooseLevel()
        {
            await ChooseLevelasync();
        }
