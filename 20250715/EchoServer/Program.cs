...
namespace Server
{
    class Program
    {
        ...
        {
            string strData = START_WORD + data + END_WORD;
        ...
        {
            string errorString = $"{START_WORD}error {cnt} {data} {END_WORD}";
                ...
                while (true)
                {
                    ...
                    if (ParsingData(recvData, out command, out sock, out msg))
                    {
                        Console.WriteLine("[서버] 받은 데이터: IP 주소={0}, 커맨드={1}, 소켓={2}, 데이터={3}",
                           socket_list[cnt].RemoteEndPoint, command, sock, msg);

                        if (command.Equals(COMMAND[5]))
                            Console.WriteLine("[서버] 현재 연결된 접속자 수={0}", socket_list.Count);
