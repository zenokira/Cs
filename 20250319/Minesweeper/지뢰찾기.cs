...
namespace Minesweeper
{
    public partial class 지뢰찾기 : Form
    {
            ...
            else
            {
                ...
                if (MyTurnFlag)
                {
                    if (button.Enabled)
                    {
                        ...
                        if (isGameClear(Level))
                        {
                            network.SendMessage(COMMAND[5], mySocketNo, opponentNo, $"clear");
                            GameClear();
                        }
                        MyTurnFlag = false;
                    }
        ...
        private async void TestOpen(Mine[,] buttons, Point pt, int level)
        {   
            if (gameoverFlag) return;
            ...
            for (int w = 0; w < WAYS; w++)
            {
                ...
                if (btn.isCorrectMineFlag()) continue;
                if (btn.isMineHave()) { GameOver(); return; }
                if ( btn.isZeroCount() )
                {
                    await Task.Run(() => TestDetectorZeroButton(buttons, new Point(nx, ny), level));
                }
            ...
            else if (command.Equals(COMMAND[5]))
            {
                if (data.Equals("gameover"))
                {
                    ...
                    await Task.Run(() =>
                    {
                        ...
                        {
                            network.SendMessage(COMMAND[2], mySocketNo, opponentNo, "retry");
                        }
                        else
                        {
                            network.SendMessage(COMMAND[1], sendNo, sendNo, "wait");
                        }
                    });
                }
