...
namespace MinesweeperServer
{
    internal class Program
    {
        ...
        */
        ...
        {
            if (!playerMap.ContainsValue(sendplayer))
                return; // 맵에 없으면 지울 필요 없음

            if (playerRoomMap.TryGetValue(sendplayer, out Room room) && room != null)
            {
                // 방이 존재하는 경우
            ...
            playerRoomMap.Remove(sendplayer);
        ...
        {
            if (!playerMap.TryGetValue(sendNo, out Player sendPlayer)) return;
            if (!playerRoomMap.TryGetValue(sendPlayer, out Room room) || room == null) return;
            ...
            sendPlayer.DisconnectPlayer();
            playerMap.Remove(sendNo);
            playerRoomMap.Remove(sendPlayer); // 혹시 여기서도 제거 필요하다면 추가
        ...
        private void HandleClientExitAction(int sendNo)
        {
            ...
            playerMap.Remove(sendplayer.Id);
            playerRoomMap.Remove(sendplayer); // 추가: roomMap에서도 제거
        }
