...
namespace Minesweeper
{
    public partial class 지뢰찾기 : Form
    {
        ...
        GAMESTATE myState = GAMESTATE.진행전;    // 0 게임중  1 게임오버  2 클리어
        GAMESTATE opp_state = GAMESTATE.진행전;  // 0 게임중  1 게임오버  2 클리어
        ...
        private async Task HandleServerClickAsync(Mine button)
        {
            ...
            else
            {
                ...
                { 
                    await Task.Run( () =>
                    {
                        if (MessageBox.Show("승리하셨습니다. 혼자 이어서 마무리를 계속 하시겠습니까?", $"Player{mySocketNo + 1}", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
        ...
        {
            GameClear();
            if (MessageBox.Show("다시 하시겠습니까?", $"Player{mySocketNo+1}", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ChooseLevel();
                GameReset();
                GameStart(solo_level);
        ...
        {
            if (opp_timer == -1) return GAMERESULT.진행중;         // 진행중
        ...
        {
            if (oppState == GAMESTATE.게임중)
            {
                if (myState == GAMESTATE.게임오버) return GAMERESULT.패배;
                else if (myState == GAMESTATE.클리어) return GAMERESULT.승리;
            }
            else if (oppState == GAMESTATE.게임오버)
            {
                if (myState == GAMESTATE.게임오버) return GAMERESULT.무승부;
                else if (myState == GAMESTATE.클리어) return GAMERESULT.승리;
                else return GAMERESULT.보류; // 승리 판정 보류
            }
            else if (oppState == GAMESTATE.클리어)
            {
                if (myState == GAMESTATE.클리어)
                {
                    GAMERESULT timeResult = JudgeTimerResult(); // (0: 무승부, 1: 승리, 2: 패배, -1: 진행중)
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                else if (command.Equals(COMMAND[3]))
                {
                    ...
                    if (data.StartsWith("exit"))
                    {
                        ...
                        {
                            case "opponent_left":
                                if (MessageBox.Show("상대방이 접속을 종료하였습니다.\n다시 하시겠습니까? (YES or No)", $"Player{mySocketNo+1}", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                {
                                ...
                                {
                                    network.SendMessage(COMMAND[4], mySocketNo, opponentNo, "exit", "COMMAND[3] 일때");
                                    ChooseMatchingLevel();
                                }
                                break;
                        }
                ...
                else if (command.Equals(COMMAND[4]))
                {
                    ...
                    if (sendNo == opponentNo && recvNo == mySocketNo && !MyTurnFlag)
                    {
                        MyTurnFlag = true;
                        OppTurnFlag = false;
                        if (opp_state == GAMESTATE.진행전 ) opp_state = GAMESTATE.게임중;
                        lb_Turn.Text = "내 차례";
                ...
                else if (command.Equals(COMMAND[5]))
                {
                    ...
                    {
                        opp_state = GAMESTATE.게임오버;
                        return;
                    }
                    else if (data.StartsWith("clear"))
                    {
                        opp_state = GAMESTATE.클리어;
                    ...
                    else
                    {
                        if (data.Equals("solo"))
                        {
                            MessageBox.Show("싱글 모드로 전환됩니다\n 멀티 모드를 원하신다면 다시 접속해주세요", $"Player{mySocketNo + 1}");
                            ChangeSoloMode();                  
                            return;
                        }
                        else if (data.Equals("multi"))
                        {
                            MessageBox.Show("상대가 승리하였습니다\n다만 상대가 끝까지 깨기를 원하여 솔로모드로 전환되었습니다.\n매칭 대기중으로 전환합니다.", $"Player{mySocketNo + 1}");
                            network.SendMessage(COMMAND[2], mySocketNo, serverNo, "retry", "recv 에서 multi");
                            lb_Turn.Text = "매칭 대기중";
                            return;
                        }
                        ...
                        if (MessageBox.Show($"{result}\n다시 하시려면 Yes 를, 그만 하고 싶으시면 No 를 눌러주세요", $"Player{mySocketNo + 1}", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            network.SendMessage(COMMAND[2], mySocketNo, mySocketNo, "retry", "recv 에서 messagebox yes");
                        }
                        else
                        {
                            network.SendMessage(COMMAND[4], mySocketNo, opponentNo, "exit", "COMMAND[5] 일때");
                            ChooseMatchingLevel();
                        } 
                    }
                }
        ...
        private void ChooseMatchingLevel()
        {
            ...
            network.SendMessage(COMMAND[1], mySocketNo, serverNo, multi_level.ToString(), "시작하기");
            GameReset();
            GameStart(multi_level);
            lb_Turn.Text = "매칭 대기중";
        }
        ...
        void ChangeSoloMode()
        {
            if (!SoloFlag) SoloFlag = true;
            solo_level = multi_level;
            lb_Turn.Text = "싱글 모드";
            network.SendMessage(COMMAND[3], mySocketNo, serverNo, "exit", "ChangeSoloMode");
            network.Disconnect();
            network = null;
        }
    }
