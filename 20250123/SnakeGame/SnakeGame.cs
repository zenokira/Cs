    ...
    public partial class SnakeGame : Form
    {
            ...
            if (isSnakeFeeding())
            {
                ...
                if (isLevelUP())
                {
                    ...
                    else if (MessageBox.Show("레벨업", "레벨업", MessageBoxButtons.OK) == DialogResult.OK)
                    {
                        setObstacle();
                        snake.setHeadLocation(CorrectionPoint(CenterPoint));
                        snake.setHeadVector(SnakeVector.LEFT);
                        CountDown();
                    }
                }
        ...
        void GameSave()
        {
            SaveLoad sv = new SaveLoad(
               ExtractionPointToLabel(snake.GetSnakeAllLabelList()),
               ExtractionPointToLabel(obstacle.getObstacleList()),
               ExtractionPointToLabel(feed.getFeedList()),
               ConvertDictToList(useLocationDict),
               LV, Length, LVUP_point, Jumsu, snake.getVector()
               );
            sv.Save();
        }
        void GameLoad()
        {
            ...
            List<Point> s = sv.getSnakeList();
            List<Point> o = sv.getObstacleList();
            List<Point> f = sv.getFeedList();

            LoadSnake(s, sv.GetSnakeVector());
            LoadObstacle(o);
            LoadFeed(f);
            LoadLocationDict(sv.getUseList());
            LoadState(sv.getState());

            StatusTextUpdate();
            CountDown();
        }

        List<Point> ConvertDictToList(Dictionary<Point,int> dic)
        {
        ...
        void LoadLocationDict(List<Point> list)
        {
            useLocationDict.Clear();
            foreach (Point p in list)
            {
                useLocationDict.Add(p, 0);
            }
        }
        ...
        void LoadObstacle(List<Point> o)
        {
            obstacle.Clear();

            foreach (Point p in o)
            {
                obstacle.LoadObstacle(p);
            }
        }
        void LoadSnake(List<Point> s, SnakeVector vector)
        {
            snake.setHeadLocation(s.Last());
            ...
            {
                snake.LoadSnakeBody(p);
            }

            snake.setHeadVector(vector);
        }
        ...
        private void 메뉴ToolStripMenuItem_DropDownClosed(object sender, EventArgs e)
        {
            if(chckStartFlag)
                CountDown();
        }
