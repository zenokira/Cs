        ...
        void ProccessReceive(int cnt)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        if (socket_play_list.Contains(cnt)) socket_play_list.Remove(cnt);
                        if (socket_wait_list.Contains(cnt)) socket_wait_list.Remove(cnt);
                        
                        socket_list[cnt].Shutdown(SocketShutdown.Both);
                        socket_list[cnt].Close();
                        socket_list[cnt].Dispose();
                    ...
                    {
                        Send(COMMAND[6], sendNo, recvNo, data);
                    }
                }
