...
namespace MinesweeperServer
{
    internal class Program
    {
        ...
        */
        ...
        public enum SystemAction { Connect, Exit, Error, Disconnect, RoomClose };
        ...
        void HandleClientCommand(int sendNo, int recvNo, string action, string data)
        {
            ...
            switch (action)
            {
                ...
                case "Level":
                    ...
                    break;
                case "LeaveRoom":
                    HandleLeaveAction(room, sendplayer);
                    break;
            }

        }

        private void HandleLeaveAction(Room room, Player sendPlayer)
        {
            Player oppPlayer = room.GetOppPlayer(sendPlayer);

            Send(CommandType.System, serverNo, sendPlayer.Id, SystemAction.RoomClose, "Close");
            Send(CommandType.System, serverNo, oppPlayer.Id, SystemAction.RoomClose, "Close");
