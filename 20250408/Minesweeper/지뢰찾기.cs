...
namespace Minesweeper
{
    public partial class 지뢰찾기 : Form
    {
        ...
        readonly string[] COMMAND = { "start", "wait", "retry", "exit", "click", "gameover", "clear", "server_shutdown" }; // win draw lose
                ...
                else if (MyTurnFlag)
                {
                    if (button.Enabled)
                    {
                        ...
                        if (isGameClear(Level))
                        {
                            ...
                            if (opp_state == 1)
                                network.SendMessage(COMMAND[6], mySocketNo, opponentNo, "draw");
                            else if (opp_state == 2)
                                network.SendMessage(COMMAND[6], mySocketNo, opponentNo, "win");
        ...
        async void RecvDataUse(string o)
        {
            ...
            for(int i = 0; i < list.Count; i++)
            {
                ...
                else if (command.Equals(COMMAND[7]))
                {
