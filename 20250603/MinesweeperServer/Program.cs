...
namespace MinesweeperServer
{
    internal class Program
    {
        ...
        */
        ...
        public enum GameAction { Click, Result, FirstClick, MineMap }
        ...
        void SetInitGame(Room room)
        {
            ...
            SendInitGame(room, player1,player2, strMine);
            SendInitGame(room, player2, player1, strMine);


            Console.WriteLine($"[LOG] player1 : {player1} player2 : {player2} Mine : {strMine}");
        }

        void SendInitGame(Room room, int sendNo, int recvNo, string mine )
        {
            Point firstclick = room.FirstClick;
 
            Send(CommandType.Server, sendNo, recvNo, ServerAction.OpponentInfo, "");
            Send(CommandType.Server, sendNo, recvNo, ServerAction.TurnInfo, $"{room.FirstTurnPlayer.Id}");
            Send(CommandType.Game, sendNo, recvNo, GameAction.MineMap, mine);
            Send(CommandType.Game, serverNo, sendNo, GameAction.FirstClick, $"{firstclick.X}*{firstclick.Y}");
        }
