    ...
    internal class Program
    {
        ...
        void end()
        {
            ...
            foreach (int recv in socket_list.Keys.ToList()) // ToList()로 복사본 사용
            {
                try
                {
                    Send(COMMAND[6], serverCnt, recv, "서버종료");
                }
                catch (Exception ex)
                {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        {
                            Console.WriteLine($"[LOG] socket_play_list : {socket_play_list.Count}  socket_wait_list : {socket_wait_list.Count}  socket_list : {socket_list.Count}");
                        ...
                        else if (command.Equals(COMMAND[5]))
                        {
                            ...
                            if (data.Equals("win"))
                            {
                                Send(COMMAND[5], serverCnt, sendNo, data);
