        ...
        void ProcessClient(int cnt)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    string[] sliceData = new string[3];
                    if (ParsingData(recvData, out sliceData[0], out sliceData[1], out sliceData[2]))
                    {
                        int n = Convert.ToInt32(sliceData[1]);
                        if (socket_list.ContainsKey(n)) Send(recvData, n);
                        else
                        {
                            Send(recvData, cnt);
                        }
                    ...
                    if (isWhatCommand(sliceData[0]) == 0)
                    {
