    ...
    public partial class 지뢰찾기 : Form
    {
        ...
        private async Task HandleServerClickAsync(Mine button)
        {
            ...
            if (button.isMineHave())
            {
                ...
                if (first)
                {
                    network.SendMessage(COMMAND[5], mySocketNo, opponentNo, "gameover", "핸들 마인찾기");
                    MessageBox.Show("지뢰를 터트리셨습니다. 상대방 턴이 종료될 때까지 기다려주세요.");
                }
                else
                {
                    gameResult = JudgeGameResult(myState, opp_state);
                }
                SafeInvoke(this, () =>GameOver());
            }
            else if (button.isZeroCount())
            {
                myState = 0;
        ...
        private async Task MultiGameClearAsync()
        {
            ...
            if (first)
            {
                network.SendMessage(COMMAND[5], mySocketNo, opponentNo, $"clear {timer}", "MultiGameClear");
                timer_PlayTime.Stop();
        ...
        private void SendGameResult(int result)
        {
            ...
            {
                case 0:
                    network.SendMessage(COMMAND[5], mySocketNo, opponentNo, "draw", "SendGameResult");
                    break;
                case 1:
                    network.SendMessage(COMMAND[5], mySocketNo, opponentNo, "win", "SendGameResult");
                    break;
                case 2:
                    network.SendMessage(COMMAND[5], mySocketNo, opponentNo, "lose", "SendGameResult");
                    break;
            }
        }
        ...
        private void 기권하기ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (mySocketNo != -1 && opponentNo != -1)
                network.SendMessage(COMMAND[5], mySocketNo, opponentNo, "retire", "기권하기");
        ...
        private void 지뢰찾기_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                if (first)
                {
                    if (myState == 1)
                    {
                        network.SendMessage(COMMAND[5], mySocketNo, opponentNo, "gameover", "마우스클릭 first gameResult1");
                        GameOver();
                    }
                    else if (myState == 2)
                    {
                        network.SendMessage(COMMAND[5], mySocketNo, opponentNo, $"clear {timer}","마우스클릭 first gameResult2");
                        GameClear();
                    }
                }
                else
                {
                    int gameResult = JudgeGameResult(myState, opp_state);
                    SendGameResult(gameResult);
                }
            }
        }
