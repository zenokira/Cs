    ...
    public partial class GameMain : Form
    {
        ...
        private void LoadListViewFromJson()
        {
            ...
            try
            {
                ...
                record.ListView_Clear();

                if (data == null) return;

                // 움직인횟수 오름차순 정렬
                var ordered = data.OrderBy(d => int.Parse(d["움직인횟수"].ToString() ?? "0")); // linq 다시 한번 보기
                ...
                foreach (var d in ordered)
                {
                    ...
                    int moves = int.TryParse(d["움직인횟수"]?.ToString(), out var m) ? m : 0;
                    ...
                    var movesSub = new ListViewItem.ListViewSubItem(item, moves.ToString()) { };
        ...
        private void timer_save_Tick(object sender, EventArgs e)
        {
            ...
            int current = record.GetListViewCount();

            if (current == lastSavedCount) return;

            try
            {
                SaveListViewToJson();
                lastSavedCount = record.GetListViewCount();
            }
            catch(Exception ex)
            {
