...
namespace _2048
{
    class GamePan
    {
        const int COLS = 4;
        const int ROWS = 4;
        const int LINE_CNT = 5;
        const int MAX_TILE_CNT = 16;
        Point GamePan_StartPoint;
        Rectangle GamePan_Rect;
        Size GamePan_Size;
        Point[,] Tile_Point = new Point[ROWS, COLS];

        public Rectangle col_Rect;
        public Rectangle row_Rect;
        

        NumberTile[,] numberTiles = new NumberTile[ROWS, COLS]; 
        public int GAMEPAN_GAP { get; } = 10;
        public Size SIZE { get; set; }
        public Point STARTPOINT { get; set; }
        public Rectangle RECT { get; set; }

        public Rectangle SCORE_RECT { get; set; }
      

        public int[] row = new int[LINE_CNT];
        public int[] col = new int[LINE_CNT];
        ...
        private void Initialize()
        {
            ...
            boardBmp = new Bitmap(SIZE.Width, SIZE.Height);
        ...
        private void ReadyToBackImage()
        {
            ...
            using (SolidBrush bgBrush = new SolidBrush(Color.Wheat))
        ...
        public void NewTile()
        {
            ...
            while (flag)
            {
                ...
                {
                    int idx = random.NextDouble() < 0.75 ? 0 : 1;
                    numberTiles[y, x] = new NumberTile(y, x, true, idx);
                    flag = false;
                }
            }
        }
            ...
            for (int y = 0; y < numberTiles.GetLength(0); y++)
            {
                for (int x = 0; x < numberTiles.GetLength(1); x++)
                {
                    ...
                    if (tile != null)
                    {
                        ...
                        g.DrawImage(tile.DrawTile(), drawX, drawY, NumberTile.TILESIZE, NumberTile.TILESIZE);
                    }
                }
            }
        }
        ...
        public void MoveTile(Keys key, out bool moved)
        {
            moved = false;

            (int dx, int dy, Action moveAction)? action = key switch
            {
        ...
        private void MoveTileLine(List<NumberTile> tileList, int lineLength, bool reverse)
        {
            ...
            var mergedList = new List<NumberTile>();
            ...
            for (int i = 0; i < tileList.Count; i++)
            {
                ...
                // 다음 타일이 병합 가능한 경우
                if (i + 1 < tileList.Count && current.GetNumber() == tileList[i + 1].GetNumber())
                {
                    current.NextLevel();
            ...
            // 빈 칸 채우기
            while (mergedList.Count < lineLength)
                mergedList.Add(null);

            if (reverse) mergedList.Reverse();

            // 원래 리스트 갱신
            tileList.Clear();
            tileList.AddRange(mergedList);
        }
